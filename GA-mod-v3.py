...
def generate_offspring(parent_population, candidate_points, max_budget):
    """
    生成子代
    新策略：
    1. 20% 的优秀父代直接保留到子代
    ...
    elite_parents = sorted_parents[:num_elite]
    offspring_population.extend(elite_parents)
    
    # 2. 从优秀父代中生成剩余子代
    # 40% 保留位置信息
    num_location_preserved = int(population_size * 0.4)
    for _ in range(num_location_preserved):
        parent = random.choice(elite_parents)
        new_individual = []
        current_budget = 0
        
        for location, _ in parent:
            remaining_budget = max_budget - current_budget
            available_scales = [s for s in SCALES if SCALE_BUDGET[s] <= remaining_budget]
            
            if available_scales:
                new_scale = random.choice(available_scales)
                current_budget += SCALE_BUDGET[new_scale]
                new_individual.append((location, new_scale))
        
        if len(new_individual) == PARAMS['num_selected_points']:
            offspring_population.append(new_individual)
        else:
            new_individual = initialize_individual(candidate_points, PARAMS['num_selected_points'], max_budget)
            if new_individual:
                offspring_population.append(new_individual)
    
    # 40% 保留规模信息
    num_scale_preserved = int(population_size * 0.4)
    for _ in range(num_scale_preserved):
        parent = random.choice(elite_parents)
        new_individual = []
        current_budget = 0
        scales = [scale for _, scale in parent]
        
        for scale in scales:
            if current_budget + SCALE_BUDGET[scale] <= max_budget:
                new_location = random.choice(candidate_points)
                current_budget += SCALE_BUDGET[scale]
                new_individual.append((new_location, scale))
    ...
    # 如果子代数量不足，从优秀父代中随机选择补充
    while len(offspring_population) < population_size:
        offspring_population.append(random.choice(elite_parents))
...
def genetic_algorithm(candidate_points, converted_stores, population_density, pixel_width, pixel_height,
    ...
    for generation in pbar:
        ...
        # 生成新一代
        population = generate_offspring(population, candidate_points, max_budget)
    ...
    # 保存统计数据到文件
    with open('fitness_history-23.txt', 'w', encoding='utf-8') as f:
...
try:
    ...
    # 保存结果
    with open('best_candidates-23-v3.txt', 'w', encoding='utf-8') as f:
